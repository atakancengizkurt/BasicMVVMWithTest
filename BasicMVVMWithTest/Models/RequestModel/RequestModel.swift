...
class RequestModel {
    // MARK: Properties
    ///Path of your request endpoint
    var path : String {
        ""
    }
    
    ///Parameters that you want to add to your reques turl
...
extension RequestModel{
    ...
    private func appendBodyIfNeeded(to request: URLRequest) -> URLRequest {
        ...
        if let data = try? JSONSerialization.data(withJSONObject: parameters, options: []), let encodedJson = String(data: data, encoding: .utf8) {
